...
class ServiceRequest(BaseModel):
    ...
    due_date: date | None = Field(
        description="The calendar due date stated in the message, or None if no specific calendar date is stated."
    )
    ...
    quote: str = Field(
        max_length=300,
        description="Copy a short span from the message verbatim to support the urgency decision. Do not paraphrase or alter the wording."
    )
...
SYSTEM_ZERO_SHOT = """\
Extract the service request from the message.

Messages may be in English, French, or German.

For category, use only: access, hardware, billing, facilities, other.
For urgency, use only: urgent, standard, info.

For due_date, return the specific calendar date if one is explicitly
stated in the message. If the message gives only a relative expression
such as "before the end of the month", do not convert it into a date;
return None. If no date is stated, return None.

For quote, copy a short span from the message character for character
that supports your urgency decision. The quote must appear verbatim in
the original message. Do not translate, paraphrase, normalize, or alter
the wording.
"""
