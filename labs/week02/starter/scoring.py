...
def score_one(record, gold, document_text: str) -> dict[str, FieldResult]:
    ...
    return {
        ...
        "due_date": FieldResult(
            correct=( record.due_date is None and gold.due_date is None) 
            or (
                record.due_date is not None
                and gold.due_date is not None
                and record.due_date.isoformat() == gold.due_date
            ),
            got=record.due_date,
            expected=gold.due_date,
        ),
        "quote": FieldResult(
            correct=record.quote in document_text,
            got=record.quote,
            expected=None,
        ),
    }
